...
public class ItemTooltipEvents {
    public static void init(ItemStack stack, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (stack.getItem() instanceof ComponentItem
                ...
                && Registry.ITEM.getKey(stack.getItem()).getNamespace().equalsIgnoreCase(UltimineAddition.MOD_ID)) {
            if (ConfigHandler.COMMON.PLAYSTYLE_MODE.get() == PlaystyleMode.LEGACY) {
                tooltipComponents.add(1, new TranslatableComponent("tooltip.ultimine_addition.legacy_mode.disabled_item").withStyle(ChatFormatting.RED));
            }
        }

        if (stack.getItem() instanceof PotionItem && PotionUtils.getPotion(stack) instanceof MineGoPotion potion) {
            MutableComponent name = tooltipComponents.get(0).copy().append(new TextComponent(" | ").withStyle(ChatFormatting.DARK_GRAY).append(potion.getComponentType().get()));
            tooltipComponents.set(0, name);

            tooltipComponents.add(1, new TextComponent("• ").withStyle(ChatFormatting.DARK_GRAY).append(new TranslatableComponent("tooltip.ultimine_addition.skill_card.tier", potion.getTier().getDisplayName())).withStyle(ChatFormatting.ITALIC));
            if (ConfigHandler.COMMON.PLAYSTYLE_MODE.get() == PlaystyleMode.LEGACY) {
                tooltipComponents.add(1, new TranslatableComponent("tooltip.ultimine_addition.legacy_mode.disabled_item").withStyle(ChatFormatting.RED));
            }
        }
