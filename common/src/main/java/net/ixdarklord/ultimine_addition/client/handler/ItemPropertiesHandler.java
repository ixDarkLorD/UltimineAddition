...
public class ItemPropertiesHandler {
    public static void register() {
        ...
        for (MiningSkillCardItem.Type type : MiningSkillCardItem.Type.TYPES) {
            ...
            ItemPropertiesRegistry.register(item, new ResourceLocation(Constants.MOD_ID, "tier_2"), (stack, level, living, id) -> MiningSkillCardItem.isTierEqual(stack, MiningSkillCardItem.Tier.Apprentice) ? 1.0F : 0.0F);
            ItemPropertiesRegistry.register(item, new ResourceLocation(Constants.MOD_ID, "tier_3"), (stack, level, living, id) -> MiningSkillCardItem.isTierEqual(stack, MiningSkillCardItem.Tier.Adept) ? 1.0F : 0.0F);
            ItemPropertiesRegistry.register(item, new ResourceLocation(Constants.MOD_ID, "tier_maxed"), (stack, level, living, id) -> MiningSkillCardItem.isTierEqual(stack, MiningSkillCardItem.Tier.Mastered) ? 1.0F : 0.0F);
        }
    }
}
