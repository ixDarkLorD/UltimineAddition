...
public class SkillsRecordDebugCommand {
    public static void register(CommandDispatcher<ClientCommandRegistrationEvent.ClientCommandSourceStack> dispatcher, CommandBuildContext ignored) {
        FTBUltimineAddition.withClientCommandPrompt(dispatcher, 2, (builder) -> builder.then(ClientCommandRegistrationEvent.literal("skills_record").then(ClientCommandRegistrationEvent.literal("debug_mode").then(ClientCommandRegistrationEvent.argument("state", BoolArgumentType.bool()).executes((context) -> setEditMode(context.getSource(), BoolArgumentType.getBool(context, "state")))))));
    }
