...
public final class MCIngredient implements Predicate<ItemStack> {
   ...
   private void dissolve() {
      ...
      this.tier = Arrays.stream(this.values).map(Value::tier).toList().get(0);
   }
   ...
   public static NonNullList<Ingredient> toNormal(NonNullList<MCIngredient> inputs) {
         ...
         ItemStack[] items = Arrays.stream(ingredient.getItems()).peek((stack) -> {
            ...
            if (item2 instanceof MiningSkillCardItem item) {
               if (ingredient.getTier() != null) {
                  item.getData(stack).setTier(ingredient.getTier()).save();
               }
            }
      ...
      result.addAll(inputs.stream().filter((ingredient) -> !Arrays.stream(ingredient.values).filter((value) -> value instanceof TagValue).toList().isEmpty()).map((ingredient) -> {
         Stream<Ingredient.TagValue> tagValueStream = Arrays.stream(ingredient.values).filter((value) -> value instanceof TagValue).map((value) -> new Ingredient.TagValue(((TagValue) value).tag));
   ...
   record TagValue(TagKey<Item> tag, MiningSkillCardItem.Tier tier) implements Value {
      ...
      public Collection<ItemStack> getItems() {
            ...
            for (Holder<Item> holder : BuiltInRegistries.ITEM.getTagOrEmpty(this.tag)) {
               ...
               if (this.tier != null && holder instanceof MiningSkillCardItem item) {
                  item.getData(stack).setTier(this.tier).save();
               }
      ...
      public JsonObject serialize() {
         ...
         if (this.tier() != null) {
            jsonObject.addProperty("tier", this.tier().getValue());
         }
   ...
   static class ItemValue implements Value {
      ...
      ItemValue(ItemStack stack, MiningSkillCardItem.@Nullable Tier tier) {
         this.stack = stack;
         this.tier = tier;
      }
      ...
      public JsonObject serialize() {
         ...
         MiningSkillCardItem.Tier tier = this.tier();
         if (tier != null) {
            jsonObject.addProperty("tier", tier.getValue());
         }
