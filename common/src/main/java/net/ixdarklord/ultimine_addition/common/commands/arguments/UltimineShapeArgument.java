...
public class UltimineShapeArgument implements ArgumentType<Shape> {
    ...
    private static final DynamicCommandExceptionType ERROR_UNKNOWN_SHAPE = new DynamicCommandExceptionType((shape) -> Component.translatable("argument.ultimine_addition.ultimine_shape.unknown", shape));
    ...
    public Shape parse(StringReader reader) throws CommandSyntaxException {
        ...
        Optional<Shape> shape = FTBUltimineIntegration.getShapesList().stream().filter((s) -> string.equals(s.getName())).findFirst();
        if (shape.isEmpty()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        List<String> shapesId = FTBUltimineIntegration.getShapesList().stream().map(Shape::getName).toList();
