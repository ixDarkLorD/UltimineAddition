    ...
    public void tick() {
        if (this.legacy != null) {
            if (this.legacy.getRequiredAmount() == 0) {
                ...
                int value = new LegacyRandomSource(RandomSupport.seedUniquifier()).nextIntBetweenInclusive(min, max);
    ...
    public static class Legacy {
        ...
        public void createInfoComponent(List<Component> tooltipComponents, boolean isShiftPressed) {
            if (!data.stack.hasTag()) {
                if (isShiftPressed)
                    tooltipComponents.add(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.info"));
                return;
            }

            if (!isShiftPressed) {
                if (!data.isAccomplished)
                    tooltipComponents.add(1, createBrackets(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.sealed").withStyle(ChatFormatting.GRAY)));
                else
                    tooltipComponents.add(1, createBrackets(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.opened").withStyle(ChatFormatting.GOLD)));
            } else {
                if (!data.isAccomplished) {
                    ChatFormatting formatting = ChatFormattingUtils.get3ColorPercentageFormat(minedBlocks, requiredAmount);
                    Component component = new TextComponent(String.valueOf(minedBlocks)).withStyle(formatting);
                    tooltipComponents.add(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.quest.info", requiredAmount).withStyle(ChatFormatting.DARK_AQUA));
                    tooltipComponents.add(new TextComponent("➤ ").withStyle(ChatFormatting.DARK_GRAY).append(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.quest", component).withStyle(ChatFormatting.GRAY)));
                } else tooltipComponents.add(new TranslatableComponent("tooltip.ultimine_addition.certificate.legacy.quest.congrats", minedBlocks).withStyle(ChatFormatting.GREEN));
            }
        }

        @SuppressWarnings("UnnecessaryUnicodeEscape")
        private Component createBrackets(Component component) {
            return new TextComponent("『").withStyle(ChatFormatting.DARK_GRAY).append(component).append(new TextComponent("\u300F").withStyle(ChatFormatting.DARK_GRAY));
        }
