...
public class CardsCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher, Commands.CommandSelection ignored) {
    ...
    private static int updateChallengeValue(CommandSourceStack source, @NotNull Collection<ServerPlayer> targets, Pair<ResourceLocation, ChallengesData> cData, int slotIndex, int amount, Boolean replaceValue) throws CommandSyntaxException {
        int i = 0;
            ...
            if (stack.getItem() instanceof MiningSkillCardItem card && card.getData(stack).isChallengeExists(cData.getFirst())) {
                ...
                if (amount > -1) {
                    if (replaceValue) {
                        ...
                        source.sendFailure(new TranslatableComponent("command.ultimine_addition.challenge.accomplished", cData.getFirst().toString()).withStyle(ChatFormatting.RED));
                        i++;
                        return i;
                    }
                } else {
                ...
                if (player == source.getPlayerOrException()) {
                    source.sendSuccess(new TranslatableComponent("command.ultimine_addition.challenge.success", cData.getFirst().toString(), oldValue, newValue).withStyle(ChatFormatting.DARK_AQUA), true);
                }
                if (i > 1 && player != source.getPlayerOrException() && !player.hasPermissions(2)) {
                    player.displayClientMessage(new TranslatableComponent("command.ultimine_addition.challenge.receiver", cData.getFirst().toString(), oldValue, newValue, Objects.requireNonNull(source.getPlayerOrException()).getName().getString()).withStyle(ChatFormatting.GRAY), false);
                }
                if (i > 1) {
                    source.sendSuccess(new TranslatableComponent("command.ultimine_addition.challenge.sender", cData.getFirst().toString(), oldValue, newValue).withStyle(ChatFormatting.GRAY), true);
                    int x = 1;
                ...
                source.sendFailure(new TranslatableComponent("command.ultimine_addition.cards.not_found").withStyle(ChatFormatting.RED));
                i++;
            }
        }
        return i;
    ...
    private static int setTier(CommandSourceStack source, @NotNull Collection<ServerPlayer> targets, MiningSkillCardItem.Tier tier, int slot) throws CommandSyntaxException {
        int i = 0;
        for (ServerPlayer player : targets) {
            ...
            if (stack.getItem() instanceof MiningSkillCardItem card && card.getData(stack).getTier() != tier) {
                ...
                if (player == source.getPlayerOrException()) {
                    source.sendSuccess(new TranslatableComponent("command.ultimine_addition.cards.tier.set.success", stack.getHoverName(), tier.name()).withStyle(ChatFormatting.DARK_AQUA), true);
                }
                if (i > 1 && player != source.getPlayerOrException() && !player.hasPermissions(2)) {
                    player.displayClientMessage(new TranslatableComponent("command.ultimine_addition.cards.tier.set.receiver", stack.getHoverName(), tier.name(), Objects.requireNonNull(source.getPlayerOrException()).getName().getString()).withStyle(ChatFormatting.GRAY), false);
                }
                if (i > 1) {
                    source.sendSuccess(new TranslatableComponent("command.ultimine_addition.cards.tier.set.sender", stack.getHoverName(), tier.name()).withStyle(ChatFormatting.GRAY), true);
                    int x = 1;
                    for (ServerPlayer p : targets) {
                        if (p != source.getPlayerOrException()) {
                            source.sendSuccess(new TextComponent(x + ": " + p.getName().getString()).withStyle(ChatFormatting.YELLOW), true);
                ...
                source.sendFailure(new TranslatableComponent("command.ultimine_addition.cards.tier.set.already_setted", stack.getHoverName().getString().toLowerCase(), tier.name()).withStyle(ChatFormatting.RED));
                i++;
            } else if (player == source.getPlayerOrException()) {
                source.sendFailure(new TranslatableComponent("command.ultimine_addition.cards.not_found").withStyle(ChatFormatting.RED));
                i++;
