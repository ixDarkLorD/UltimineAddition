...
public abstract class ItemModelProvider extends ModelProvider<ItemModelBuilder> {
    ...
    protected ItemModelBuilder simpleItem(ResourceLocation location, String string, boolean addSuffixToName) {
        ...
        return this.getBuilder(this.addSuffixToResourceLocation(location, addSuffixToName ? string : null).toString()).parent(new ModelFile.UncheckedModelFile("item/generated")).texture("layer0", ResourceLocation.fromNamespaceAndPath(location.getNamespace(), path));
    }
    ...
    protected ItemModelBuilder handheldItem(ResourceLocation location, String string) {
        ItemModelBuilder itemModelBuilder = this.getBuilder(this.addSuffixToResourceLocation(location, string).toString()).parent(new ModelFile.UncheckedModelFile("item/handheld"));
        String string2 = location.getNamespace();
        ResourceLocation resourceLocation = this.addSuffixToResourceLocation(location, string);
        return itemModelBuilder.texture("layer0", ResourceLocation.fromNamespaceAndPath(string2, "item/" + resourceLocation.getPath()));
    }
    ...
    protected ItemModelBuilder specialRendererItem(ResourceLocation location, String string) {
        return this.getBuilder(this.addSuffixToResourceLocation(location, string).toString()).parent(new ModelFile.UncheckedModelFile("builtin/entity")).guiLight(GuiLight.FRONT);
    }

    private ResourceLocation addSuffixToResourceLocation(ResourceLocation location, String string) {
        ResourceLocation rl = location;
        if (string != null) {
            String string2 = location.toString();
            rl = ResourceLocation.parse(string2 + string);
        }
